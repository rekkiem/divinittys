...
import { created, badRequest, serverError, slugify } from '@/lib/utils/api';
...
export async function POST(req: NextRequest) {
  ...
  try {
    const parsed = z.object({ name: z.string().min(1), slug: z.string().min(1).optional() }).parse(await req.json());
